...
with open(r'Files\comments.txt', 'r') as f:
    foo = [line.strip() for line in f]

# keyword
with open(r'Files\keywords.txt', 'r') as f:
    foooo = [line.strip() for line in f]

keywords = open(r'Files\keywords.txt', 'r')
x = 10
while x < 20:
    for line in keywords:
        scrape(line)

        with open(r"Files\links.txt", 'r+') as f:
            f.readline()
            data = f.read()
            f.seek(0)
            f.write(data)
